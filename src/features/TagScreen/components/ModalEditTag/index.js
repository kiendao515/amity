...
ModalEditTag.propTypes = {
    tagItem: PropTypes.object,
    show: PropTypes.bool,
    onClose: PropTypes.func,
};

ModalEditTag.defaultProps = {
    tagItem: null,
    show: false,
    onClose: null,
};
...
function ModalEditTag(props) {
    ...
    // MARK: --- Params ---
    const { tagItem, show, onClose } = props;
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            const params = { _id: tagItem?._id, ...values };
            console.log(params);
            try {
                const res = await dispatch(thunkEditTag(params));
                console.log(res);
                if (res.payload.result === "success") {
                    handleClose();
                    formik.handleReset();
                    ToastHelper.showSuccess("Chỉnh sửa thẻ thành công");
    ...
    useEffect(() => {
        if (tagItem) {
            formik.getFieldHelpers("name").setValue(tagItem?.name || "");
            formik
                .getFieldHelpers("description")
                .setValue(tagItem?.description || "");
        }
    }, [tagItem]);
